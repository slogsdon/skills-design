...
function parseArgs(args) {
  const opts = { dir: '.', output: null, format: 'Letter', margin: '0.9in', footer: true };
  const positional = [];
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--output' || arg === '-o') { opts.output = args[i + 1]; i += 1; }
      ...
      stdout.write(
        'Usage: node scripts/pdf-paginated.mjs <dir> [--output <dir>] [--format Letter|A4] [--margin <css>] [--no-footer]\n' +
        '\n' +
...
const FOOTER_TEMPLATE = `
  <div style="width:100%; font-family:'JetBrains Mono','SFMono-Regular',Menlo,monospace; font-size:8px; letter-spacing:0.06em; color:#62686f; padding:0 0.9in; display:flex; justify-content:space-between; align-items:center;">
...
async function main() {
  ...
  const margin = { top: opts.margin, right: opts.margin, bottom: opts.margin, left: opts.margin };
  let total = 0;
    ...
    for (const file of files) {
      ...
      try {
        ...
        await page.pdf({
          ...
          footerTemplate: opts.footer ? FOOTER_TEMPLATE : EMPTY_TEMPLATE,
        });
        total += 1;
